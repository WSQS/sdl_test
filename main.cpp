...
class UserApp : public sopho::App {
  ...
  virtual SDL_AppResult init(int argc, char **argv) override {
    // create a window
    window =
        SDL_CreateWindow("Hello, Triangle!", 960, 540, SDL_WINDOW_RESIZABLE);
    ...
    auto result = compiler.CompileGlslToSpv(
        vertex_source, shaderc_glsl_vertex_shader, "test.glsl", options);
    ...
    result = compiler.CompileGlslToSpv(
        fragment_source, shaderc_glsl_fragment_shader, "test.frag", options);
  ...
  virtual SDL_AppResult iterate() override {
...

    {
      ImGui::Begin("Editor");
      auto change =
          ImGui::DragFloat3("node1", vertices[0].position(), 0.01f, -1.f, 1.f);
      change = ImGui::DragFloat3("node2", vertices[1].position(), 0.01f, -1.f,
                                 1.f) ||
               change;
      change = ImGui::DragFloat3("node3", vertices[2].position(), 0.01f, -1.f,
                                 1.f) ||
               change;
      if (change) {
        vertexBuffer->upload(vertices, sizeof(vertices), 0);
    ...
    bufferBindings[0].buffer =
        vertexBuffer->data();     // index 0 is slot 0 in this example
